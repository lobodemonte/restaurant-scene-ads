...
```{r Optimal K Silhouette, eval = FALSE}
...
Without reducing the dimensionality of the data, 
...
# Checking for ideal k-value with silhouette analysis
...
# Checking for ideal k-value with elbow plot
tot_withinss <- map_dbl(2:10, function(k){
  model <- kmeans(scaled_NYC_Yelp_df, k)
  model$tot.withinss
})
...
#Attaching Cluster to df
scaled_NYC_Yelp_df$Cluster <- NYC_fulldim_kmeans$cluster
colnames(scaled_NYC_Yelp_df)[1:3] <- c("ReviewCount", "Ratings", "Price")

# Exporting as csv for pair plotting in Python Seaborn
write.csv(scaled_NYC_Yelp_df, paste(path, "NYC_Yelp_Clustered_k7.csv", sep = ""))

# Attaching NTACode to df
...
NYC_Yelp_cluster_propcount_fulldim <- NYC_Yelp_cluster_count_fulldim %>%
  as.matrix() %>%
  prop.table(., margin = 1) %>%
  as.matrix.data.frame() %>%
  as.data.frame()
...
tot_withinss <- map_dbl(2:10, function(k){
  model <- kmeans(NYC_Yelp_cluster_propcount_fulldim, k)
  model$tot.withinss
})
...
sil_width <- map_dbl(2:10,  function(k){
  model <- pam(x = NYC_Yelp_cluster_propcount_fulldim, k = k)
  model$silinfo$avg.width
})
...
NYC_Yelp_cluster_count_kmeans_fulldim <- kmeans(NYC_Yelp_cluster_propcount_fulldim, 3, nstart=4, iter.max=1000)
...
NYC_Yelp_cluster_propcount_fulldim$`NTA Cluster` <- NYC_Yelp_cluster_count_kmeans_fulldim$cluster
colnames(NYC_Yelp_cluster_propcount_fulldim)[1:7] <- c("Yelp_Cluster1", "Yelp_Cluster2", "Yelp_Cluster3", "Yelp_Cluster4", "Yelp_Cluster5", "Yelp_Cluster6", "Yelp_Cluster7")
...
write.csv(NYC_Yelp_cluster_propcount_fulldim, "./data/NYC_Yelp_NTA_Clustered_Profile_k2.csv")
```
